...
export default function FuelCardTab({ currentUser }: FuelCardTabProps) {
            ...
            if (t.status === 'In Corso') {
              ...
              return {
                ...
                expenses: [...t.expenses, {
                  ...
                  title: `Rifornimento ${fuelCompany} ${lit}L @ ${price}€/L${isFullTank ? ' [Pieno ✓]' : ''}`,
                  amount: lit * price,
                  category: 'Carburante',
                  date: date,
                  liters: lit,
  ...
  const handleDelete = async (logId: string) => {
    if (!currentUser?.email) return;
    if (!confirm("Vuoi davvero eliminare questo rifornimento?")) return;

    try {
      const res = await fetch(`/api/fuel-logs/${encodeURIComponent(currentUser.email)}/${logId}`, {
        method: 'DELETE'
      });
      ...
      // Sync Viceversa into local trips
      try {
        const savedTrips = localStorage.getItem('camper_trips');
        if (savedTrips) {
          let trips = JSON.parse(savedTrips);
          trips = trips.map((t: any) => ({
             ...t,
             expenses: t.expenses.filter((e: any) => e.id !== logId)
          }));
          localStorage.setItem('camper_trips', JSON.stringify(trips));
        }
      } catch(e) {}
      ...
      window.dispatchEvent(new CustomEvent('show-toast', { 
        detail: { message: `🗑️ Rifornimento rimosso!` } 
      }));
    } catch (err) {
  ...
  // Calculate km/L using consecutive full tanks if possible, otherwise simple estimation
  ...
  if (logs.length >= 2) {
    ...
    if (distanceCoved > 0 && litersBurned > 0) {
      averageConsumption = (distanceCoved / litersBurned).toFixed(1);
    }
  }
      ...
      {!loading && !error && logs.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-3 flex flex-col justify-center">
            ...
            <div className="text-lg sm:text-xl font-mono font-bold text-stone-800 dark:text-stone-100">{totalFuelCost.toFixed(2)} €</div>
          </div>
          <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-3 flex flex-col justify-center">
          ...
          <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-3 flex flex-col justify-center">
            ...
            <div className="text-lg sm:text-xl font-mono font-bold text-slate-700 dark:text-slate-200">{avgPrice} €/L</div>
          </div>
          <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-3 flex flex-col justify-center">
            ...
            <div className="flex items-baseline gap-1">
              ...
              {averageConsumption !== '---' && <span className="text-xs font-bold text-emerald-800 dark:text-emerald-400">km/L</span>}
            </div>
          </div>
        </div>
      )}
          ...
          <form onSubmit={handleAddLog} className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full bg-white border border-slate-300 py-2 pl-3 pr-8 rounded-xl text-sm font-mono font-bold focus:ring-2 focus:ring-emerald-500/20 outline-none"
                  />
                  <span className="absolute right-3 top-2 text-slate-400 font-black text-sm">€</span>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                />
                <span className="absolute right-3 top-2 text-slate-400 font-black text-sm text-[10px] mt-0.5">km</span>
              </div>
            </div>
              ...
              <div key={log.id} className="bg-white rounded-xl border border-slate-200 p-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-xs transition-all hover:border-[#3E4A35]/30">
                ...
                <div className="flex items-center justify-between sm:justify-end gap-4 w-full sm:w-auto pl-12 sm:pl-0">
                  <div className="text-right">
                    ...
                    <div className="text-[10px] text-slate-400 font-mono">{log.pricePerLiter.toFixed(3)} €/L</div>
                  </div>
                  <button
