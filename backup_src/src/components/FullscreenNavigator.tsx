...
out center;`;
  ...
  const speakInstruction = (text: string) => {
    ...
    try {
      if (typeof window !== 'undefined' && 'speechSynthesis' in window) {
        ...
        utterance.onerror = (e) => {
          console.warn("SpeechSynthesis error (safe to ignore on mobile background): ", e);
        };
        
        window.speechSynthesis.speak(utterance);
      }
    } catch (err) {
      console.warn("SpeechSynthesis blocked or unsupported in this sandboxed mobile browser environment:", err);
    }
  };
  ...
  }, [startLoc[0], startLoc[1], endLoc[0], endLoc[1], dest.id]);
  ...
  React.useEffect(() => {
    ...
    const map = L.map(mapContainerRef.current, {
      center: routeCoordinates[simStep] || startLoc,
      zoom: 14,
    ...
    map.on('contextmenu', (e: L.LeafletMouseEvent) => {
      ...
      window.dispatchEvent(new CustomEvent('show-toast', {
        detail: { message: `📍 Navigazione avviata per: ${pLat.toFixed(4)}, ${pLng.toFixed(4)}` }
      }));
  ...
  React.useEffect(() => {
    ...
    const POI_THRESHOLD = 30.0; // 30km as requested

    console.log('DEBUG: FullscreenNavigator useEffect - POI check starting. Places count:', places.length);
    console.log('DEBUG: Place sources:', places.map(p => p.source));
    console.log('DEBUG: Route coords length:', routeCoordinates.length);

    places.forEach(place => {
      let minDistance = Infinity;
      const isNearRoute = routeCoordinates.some(coord => {
          // Bounding box heuristic for performance (~30km or 0.27 degrees lat/lng)
          if (Math.abs(place.lat - coord[0]) > 0.5 || Math.abs(place.lng - coord[1]) > 0.5) {
              return false;
          }
      ...
      if (isNearRoute) {
        console.log('DEBUG: Found POI near route:', place.name);
        const poiIcon = L.divIcon({
          className: 'poi-map-icon',
          html: `<div style="background-color: #ef4444; width: 14px; height: 14px; border-radius: 50%; border: 2px solid white; box-shadow: 0 2px 5px rgba(0,0,0,0.4);"></div>`,
          iconSize: [14, 14],
          iconAnchor: [7, 7]
        });
        
        const popupContent = `
          <div style="font-family: system-ui, -apple-system, sans-serif; font-size: 12px; color: #1e293b; max-width: 180px; padding: 4px;">
            <p style="margin: 0; font-weight: 700; color: #ef4444; font-size: 10px; text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 2px;">Struttura sul percorso</p>
            <h4 style="margin: 0 0 3px 0; font-size: 13px; font-weight: bold; line-height: 1.2;">${place.name}</h4>
            <p style="margin: 0; color: #64748b; font-size: 10px;">${place.address || 'Sosta sintonizzata'}</p>
            ${place.priceInfo ? `<p style="margin: 4px 0 0 0; font-weight: 600; font-size: 10px; color: #0284c7; background: #f0f9ff; display: inline-block; padding: 1px 4px; border-radius: 4px;">${place.priceInfo}</p>` : ''}
          </div>
        `;

        const marker = L.marker([place.lat, place.lng], { icon: poiIcon })
          .addTo(map)
          .bindPopup(popupContent, { closeButton: false, offset: L.point(0, -4) });

        poiMarkersRef.current.push(marker);
      } else {
        console.log(`DEBUG: POI ${place.name} not found near route. Min distance: ${minDistance.toFixed(2)}km`);
      }
    });
  }, [routeCoordinates, places]);
  ...
  React.useEffect(() => {
    ...
    // Clear old markers
    osmObstacleMarkersRef.current.forEach(m => m.remove());
  ...
  React.useEffect(() => {
    ...
    // Pan map smoothly to follow camper
    map.panTo(targetCoords);
  }, [simStep, userLocation, isGPSEnabled, routeCoordinates, directionsSequence]);
