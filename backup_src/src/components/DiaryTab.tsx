...
import { Trip, DiaryExpense, DiaryPhoto } from '../types';
import { 
  BookOpen, 
  Plus, 
  Trash2, 
...
const PHOTO_PRESETS = [
  { name: 'Tramonto in Maremma 🌅', url: 'https://images.unsplash.com/photo-1523987355122-c348ebef72d4?auto=format&fit=crop&q=80&w=600' },
  { name: 'Dolomiti del Cadore 🏔️', url: 'https://images.unsplash.com/photo-1504280390367-361c6d9f38f4?auto=format&fit=crop&q=80&w=600' },
  { name: 'Costa d\'Argento 🌊', url: 'https://images.unsplash.com/photo-1510312305653-8ed496efae75?auto=format&fit=crop&q=80&w=600' },
  { name: 'Campeggio Sotto le Stelle ✨', url: 'https://images.unsplash.com/photo-1478131143081-80f7f84ca84d?auto=format&fit=crop&q=80&w=600' },
];
...
const INITIAL_TRIPS: Trip[] = [
  {
    ...
    status: 'Completato',
    expenses: [
      { id: 'te1', title: 'Gasolio Eni Siena', amount: 55.00, category: 'Carburante', date: '2026-10-10' },
      { id: 'te2', title: 'Sosta Pienza comunale', amount: 12.00, category: 'Sosta', date: '2026-10-11' },
      { id: 'te3', title: 'Pranzo Tipico Trattoria', amount: 48.00, category: 'Cibo', date: '2026-10-11' }
    ],
    photos: [
      { id: 'tp1', url: 'https://images.unsplash.com/photo-1523987355122-c348ebef72d4?auto=format&fit=crop&q=80&w=600', description: 'Il nostro amato mansardato immerso nell\'abbraccio dorato dei cipressi toscani.', date: '2026-10-11' }
    ]
  }
];
...
export default function DiaryTab({ currentUser }: DiaryTabProps) {
  const [trips, setTrips] = React.useState<Trip[]>(() => {
    const saved = localStorage.getItem('camper_trips');
    if (saved) {
      let parsedTrips = JSON.parse(saved);
      // Migration to remove the old t2 example and update t1
      parsedTrips = parsedTrips.filter((t: Trip) => t.id !== 't2');
      parsedTrips = parsedTrips.map((t: Trip) => {
        if (t.id === 't1' && t.title === 'Weekend d\'Autunno in Val d\'Orcia') {
          return {
            ...t,
            title: 'ESEMPIO: Weekend d\'Autunno in Val d\'Orcia',
            description: 'Questo è un viaggio di esempio per mostrarti come funziona il diario. Puoi modificarlo o cancellarlo in qualsiasi momento.'
      ...
      return parsedTrips.length > 0 ? parsedTrips : INITIAL_TRIPS;
    }
  ...
  React.useEffect(() => {
    ...
    window.dispatchEvent(new CustomEvent('trip-status-changed', {
      detail: { hasActiveTrip: hasActive }
    }));
  }, [trips]);
  ...
  const [diarySubTab, setDiarySubTab] = React.useState<'list' | 'details'>('list');
  ...
  const [newStatus, setNewStatus] = React.useState<Trip['status']>('Completato');
  ...
  const [expenseCategory, setExpenseCategory] = React.useState<DiaryExpense['category']>('Carburante');
  ...
  const [expenseSubMode, setExpenseSubMode] = React.useState<'general' | 'refuel'>('general');

  // New Photo Form state
  const [photoDesc, setPhotoDesc] = React.useState('');
  const [photoPresetUrl, setPhotoPresetUrl] = React.useState(PHOTO_PRESETS[0].url);
  const [photoCustomUrl, setPhotoCustomUrl] = React.useState('');
  const [photoType, setPhotoType] = React.useState<'upload' | 'preset' | 'url'>('upload');
  ...
  const [editStatus, setEditStatus] = React.useState<Trip['status']>('Completato');
  ...
  const [selectedLightboxPhotoIndex, setSelectedLightboxPhotoIndex] = React.useState<number | null>(null);
  ...
  const fuelStats = React.useMemo(() => {
    if (!activeTrip) return { totalFuelCost: 0, totalLiters: 0, avgPricePerLiter: 0, tripDistance: 0, kmPerLiter: null };
    ...
    if (totalLiters > 0) {
      const totalWeightedPrice = refuels.reduce((sum, r) => sum + ((r.liters || 0) * (r.pricePerLiter || 0)), 0);
      avgPricePerLiter = totalWeightedPrice / totalLiters;
    } else {
      const sampleCost = activeTrip.expenses.filter(e => e.category === 'Carburante');
    ...
    let tripDistance = 0;
    if (activeTrip.startOdometer && activeTrip.endOdometer && activeTrip.endOdometer > activeTrip.startOdometer) {
      tripDistance = activeTrip.endOdometer - activeTrip.startOdometer;
    } else {
      const odometers = refuels.map(r => r.odometer).filter((o): o is number => o !== undefined);
  ...
  const handleDeleteTrip = (tripId: string) => {
    if (window.confirm('Vuoi davvero eliminare questo viaggio ed eliminare tutte le sue spese e foto?')) {
  ...
  const handleAddExpense = (e: React.FormEvent) => {
    ...
    const pricePerLiterNum = fuelPricePerLiter ? parseFloat(fuelPricePerLiter) : undefined;
    ...
    if (expenseSubMode === 'refuel') {
      ...
      if (isNaN(finalAmount) || finalAmount <= 0) {
        window.dispatchEvent(new CustomEvent('show-toast', {
          detail: { message: '⚠️ Errore: Inserisci dei litri e prezzo al litro validi o l\'importo totale!' }
        }));
        return;
      }

      const litersText = litersNum ? ` ${litersNum}L` : '';
      const priceText = pricePerLiterNum ? ` @ ${pricePerLiterNum}€/L` : '';
      ...
      if (isNaN(finalAmount) || finalAmount <= 0) {
        window.dispatchEvent(new CustomEvent('show-toast', {
          detail: { message: '⚠️ Inserisci un importo valido superiore a 0!' }
        }));
        return;
      }
    }
    ...
    window.dispatchEvent(new CustomEvent('show-toast', {
      detail: { message: (expenseSubMode === 'refuel' ? '⛽ Rifornimento salvato!' : '💸 Spesa aggiunta!') }
    }));
  };
  ...
  const handleDeleteExpense = (expenseId: string) => {
    ...
    if (currentUser?.email) {
      fetch(`/api/fuel-logs/${encodeURIComponent(currentUser.email)}/${expenseId}`, {
        method: 'DELETE'
  ...
  const startEditingActiveTrip = () => {
    ...
    setEditStatus(activeTrip.status);
    setEditStartOdo(activeTrip.startOdometer ? String(activeTrip.startOdometer) : '');
  ...
  const processAndUploadFile = async (file: File) => {
    ...
    if (file.size > 15 * 1024 * 1024) {
      setUploadError("L'immagine supera la dimensione massima consentita di 15 MB.");
      setIsUploading(false);
      return;
    }
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.success && data.url) {
            ...
            window.dispatchEvent(new CustomEvent('show-toast', {
              detail: { message: `📸 Foto "${file.name}" caricata con successo!` }
            }));
          } else {
          ...
          throw new Error(errData.error || `Codice di errore: ${response.status}`);
        }
      } catch (err: any) {
  ...
  const handleAddPhoto = (e: React.FormEvent) => {
    ...
    if (photoType === 'upload') {
      ...
      if (!url) {
        setUploadError("Carica prima un'immagine usando il box o seleziona un'altra modalità.");
        return;
      }
  ...
  const totalExpensesOfActive = activeTrip ? activeTrip.expenses.reduce((sum, exp) => sum + exp.amount, 0) : 0;
  const odometerDiff = activeTrip && activeTrip.startOdometer && activeTrip.endOdometer 
    ...
    <div id="diary-container" className="space-y-6">
      ...
      {diarySubTab === 'list' && !showAddTrip && (
        <div className="bg-gradient-to-r from-[#3E4A35] to-[#5A6B4E] text-white p-5 rounded-2xl shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <span className="text-white/70 text-xs font-bold uppercase tracking-wider">Memorie On The Road</span>
            ...
            <p className="text-white/80 text-xs mt-1 max-w-xl">
              Tieni traccia delle tappe indimenticabili, dei tuoi percorsi, archivia scatti fotografici panoramici e tieni sotto controllo il budget delle spese di viaggio.
            </p>
          </div>
          <button
          ...
          {showAddTrip && (
            <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm space-y-4 max-w-2xl mx-auto animate-fade-in">
              ...
              <form onSubmit={handleCreateTrip} className="space-y-4">
                <div>
                  <label className="block text-[10px] uppercase font-bold text-slate-500 mb-1">Titolo del Viaggio</label>
                  <input
                    type="text"
                    required
                    value={newTitle}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-[10px] uppercase font-bold text-slate-500 mb-1">Data Inizio</label>
                    <input
                      type="date"
                      value={newStart}
                  ...
                  <div>
                    <label className="block text-[10px] uppercase font-bold text-slate-500 mb-1">Data Fine</label>
                    <input
                      type="date"
                      value={newEnd}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-[10px] uppercase font-bold text-slate-500 mb-1">Km Start (Odo)</label>
                    <input
                      type="number"
                  ...
                  <div>
                    <label className="block text-[10px] uppercase font-bold text-slate-500 mb-1">Km End (Odo)</label>
                    <input
                      type="number"
                ...
                <div>
                  <label className="block text-[10px] uppercase font-bold text-slate-500 mb-1">Stato Viaggio</label>
                  <select
                    value={newStatus}
                    onChange={(e) => setNewStatus(e.target.value as Trip['status'])}
                  ...
                  >
                    ...
                    <option value="Pianificato">Pianificato per il futuro</option>
                  </select>
                </div>

                <div>
                  <label className="block text-[10px] uppercase font-bold text-slate-500 mb-1">Descrizione & Sogni</label>
                  <textarea
                    rows={3}
                    value={newDesc}
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 space-y-4">
              <div className="flex justify-between items-center pb-1">
                <h3 className="font-bold text-slate-800 text-xs uppercase tracking-wider flex items-center gap-1.5">
                  <BookOpen className="w-4 h-4 text-[#3E4A35]" />
                  I Tuoi Viaggi Registrati ({trips.length})
                </h3>
              </div>
              ...
              {trips.length === 0 ? (
                <div className="text-center py-12 text-slate-400 space-y-3 bg-[#F2EFE9]/10 rounded-2xl border border-dashed border-stone-200 font-sans select-none">
                  ...
                  <p className="text-sm font-bold text-[#3E4A35]/80">Nessun viaggio pianificato o completato.</p>
                  <p className="text-xs text-slate-400">Clicca &quot;Nuovo Viaggio&quot; per iniziare ad annotare sogni, foto e spese!</p>
                  <button
                    type="button"
                    ...
                    return (
                      ...
                      >
                        <div>
                          ...
                          <h4 className="font-bold text-[#2D2926] text-sm mt-2.5 line-clamp-1 group-hover:text-[#3E4A35] transition-colors">{trip.title}</h4>
                          <p className="text-[11px] text-slate-500 mt-1 italic line-clamp-2 h-10 leading-relaxed cursor-pointer">
                            {trip.description ? `“${trip.description}”` : "Nessuna descrizione o nota inserita."}
                          </p>
                        </div>
                        ...
                        <div className="flex items-center justify-between border-t border-slate-100 pt-2 mt-2">
                          ...
                          <div className="flex items-center gap-1.5">
                            <button
                              ...
                              className="p-1.5 rounded-lg text-slate-350 hover:text-red-500 hover:bg-red-50 opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer"
          ...
          <div className="flex justify-between items-center bg-[#F2EFE9]/45 border border-stone-200 rounded-2xl p-3 px-4 shadow-xs select-none">
            ...
            <div className="flex items-center gap-3">
              <span className="text-xs text-slate-500 font-bold font-mono hidden sm:inline">
                Sotto-Scheda: <span className="text-[#3E4A35] underline">{activeTrip?.title}</span>
              </span>
              <button
                type="button"
          ...
          <div className="lg:col-span-12">
            {activeTrip ? (
              ...
              <div className="border-b border-stone-100 pb-4">
                {isEditingTrip ? (
                  <form onSubmit={handleSaveTripEdit} className="space-y-4 bg-[#F2EFE9]/25 border border-stone-250 p-4 rounded-xl">
                    <div className="flex justify-between items-center pb-1.5 border-b border-stone-150">
                      <span className="text-[10px] font-black text-[#3E4A35] uppercase tracking-wider flex items-center gap-1.5">
                        <Edit3 className="w-3.5 h-3.5" /> Modifica Dettagli del Viaggio
                      </span>
                      <button
                        type="button"
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between items-start gap-4 flex-wrap">
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="text-[10px] font-black tracking-widest text-[#5A6B4E] uppercase">Diario Attivo</span>
                          
                          <select
                            value={activeTrip.status}
                            onChange={(e) => handleUpdateTripStatus(e.target.value as Trip['status'])}
                            className={`px-2 py-0.5 rounded-full text-[9px] font-black font-mono uppercase tracking-wider cursor-pointer outline-none border-none ${
                              activeTrip.status === 'Completato' ? 'bg-[#3E4A35]/10 text-[#3E4A35]' : 'bg-orange-100 text-amber-800'
                            }`}
                          >
                            <option value="Completato">Completato</option>
                            <option value="Attivo">In Corso (Attivo)</option>
                        ...
                        <h2 className="text-xl font-bold tracking-tight text-[#2D2926] mt-1">{activeTrip.title}</h2>
                      ...
                      <div className="flex gap-4">
                        <div className="bg-[#F5F2ED]/60 border border-slate-150 p-2 px-3 rounded-xl text-center">
                          <span className="text-[8px] font-bold text-slate-400 uppercase block">Distanza</span>
                          <span className="text-sm font-black text-slate-750 font-mono">{odometerDiff > 0 ? `${odometerDiff} km` : '---'}</span>
                        </div>
                        <div className="bg-[#A45C40]/10 border border-transparent p-2 px-3 rounded-xl text-center">
                          <span className="text-[8px] font-bold text-[#A45C40] uppercase block">Budget Speso</span>
                          <span className="text-sm font-black text-[#A45C40] font-mono">{totalExpensesOfActive.toFixed(2)} €</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <p className="text-xs text-slate-600 mt-3 leading-relaxed bg-[#F5F2ED]/40 p-3 rounded-xl border border-[#3E4A35]/5 italic relative group">
                      &quot;{activeTrip.description || 'Nessuna descrizione o nota inserita per questa escursione.'}&quot;
                      <button
                        onClick={startEditingActiveTrip}
                        className="absolute right-2 bottom-2 p-1.5 bg-white/90 text-[10px] font-black text-[#3E4A35] hover:bg-[#3E4A35] hover:text-white rounded border border-slate-200/50 opacity-0 group-hover:opacity-100 transition-all shadow-xs cursor-pointer"
                      >
                        Modifica nota
                      </button>
                    </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
                ...
                <div className="space-y-4">
                  {/* Toggle Selector for Spese vs Rifornimenti */}
                  <div className="flex p-1 bg-stone-100 rounded-xl border border-stone-200/30 gap-1">
                    <button
                      type="button"
                    ...
                    >
                      ...
                      Spese di Viaggio ({activeTrip.expenses.filter(e => e.category !== 'Carburante').length})
                    </button>
                    <button
                      type="button"
                    ...
                    >
                      ...
                      Rifornimenti ({activeTrip.expenses.filter(e => e.category === 'Carburante').length})
                    </button>
                  </div>
                  ...
                  {expenseSubMode === 'general' ? (
                    ...
                    <div className="space-y-4 animate-fade-in">
                      ...
                      <form onSubmit={handleAddExpense} className="p-3 bg-stone-50 rounded-xl border border-stone-100 space-y-2">
                        <div className="grid grid-cols-2 gap-2">
                          <input
                            type="text"
                            required
                          ...
                          <select
                            ...
                            onChange={(e) => setExpenseCategory(e.target.value as DiaryExpense['category'])}
                          ...
                          >
                            ...
                            <option value="Sosta">⛺ Area di Sosta / Camping</option>
                        ...
                        <button
                          type="submit"
                          className="w-full py-1.5 bg-[#A45C40]/90 hover:bg-[#A45C40] text-white rounded-lg text-[11px] font-black uppercase tracking-wider cursor-pointer"
                        >
                          Aggiungi Spesa
                        </button>
                      </form>
                      ...
                      <div className="space-y-2 max-h-[220px] overflow-y-auto pr-1">
                        {activeTrip.expenses.filter(e => e.category !== 'Carburante').length === 0 ? (
                          <p className="text-xs text-slate-400 py-6 text-center">Nessuna spesa di viaggio inserita.</p>
                        ) : (
                          activeTrip.expenses
                            .filter(e => e.category !== 'Carburante')
                            .map((exp) => (
                              <div key={exp.id} className="flex justify-between items-center p-2.5 bg-white border border-slate-100 rounded-lg hover:border-slate-200">
                                <div>
                                  <div className="flex items-center gap-1.5">
                                    <span className="text-[8px] font-black uppercase px-2 py-0.5 rounded-full bg-slate-100 text-slate-600 font-mono">
                                      {exp.category === 'Cibo' ? 'Cibo' : exp.category === 'Sosta' ? 'Sosta' : exp.category}
                                    </span>
                                    <p className="text-xs font-bold text-slate-800 line-clamp-1">{exp.title}</p>
                                  </div>
                                  <span className="text-[9px] text-slate-400 font-mono block mt-0.5">{exp.date}</span>
                                </div>
                                <div className="flex items-center gap-2">
                                  <span className="text-xs font-black text-[#A45C40] font-mono">-{exp.amount.toFixed(2)} €</span>
                                  <button
                    ...
                    <div className="space-y-4 animate-fade-in font-sans">
                      ...
                      {/* Refueling Form */}
                      <form onSubmit={handleAddExpense} className="p-3 bg-stone-50 rounded-xl border border-stone-100 space-y-3">
                        <div className="grid grid-cols-2 gap-2.5">
                          {/* Distributor Brand */}
                          <div className="col-span-2 space-y-1">
                            <label className="block text-[9px] font-black text-slate-500 uppercase tracking-widest">Brand distributore</label>
                            <select
                              value={fuelCompany}
                              required
                            ...
                            >
                              ...
                              <option value="Q8">🔵 Q8 (Kuwait Petroleum)</option>
                              ...
                              <option value="Pompa Bianca">⚪ Pompa Bianca (No-Brand)</option>
                          ...
                          <div className="space-y-1">
                            <label className="block text-[9px] font-black text-slate-500 uppercase tracking-widest">Litri (L)</label>
                            <input
                              type="number"
                              step="0.01"
                              required
                              ...
                              onChange={(e) => {
                                ...
                                if (fuelPricePerLiter) {
                                  const computed = (parseFloat(liters) || 0) * (parseFloat(fuelPricePerLiter) || 0);
                                  setExpenseAmount(computed > 0 ? computed.toFixed(2) : '');
                                }
                              }}
                          ...
                          <div className="space-y-1">
                            <label className="block text-[9px] font-black text-slate-500 uppercase tracking-widest">Prezzo al litro (€/L)</label>
                            <input
                              type="number"
                              step="0.001"
                              required
                              ...
                              onChange={(e) => {
                                ...
                                if (fuelLiters) {
                                  const computed = (parseFloat(fuelLiters) || 0) * (parseFloat(price) || 0);
                                  setExpenseAmount(computed > 0 ? computed.toFixed(2) : '');
                                }
                              }}
                              className="w-full text-xs px-2.5 py-1.5 rounded-lg border border-slate-200 outline-none focus:border-emerald-500 text-slate-800 font-bold font-mono"
                          ...
                          <div className="space-y-1">
                            <label className="block text-[9px] font-black text-slate-500 uppercase tracking-widest">Importo Speso (€)</label>
                            <input
                              type="number"
                              step="0.01"
                              required
                          ...
                          <div className="space-y-1">
                            <label className="block text-[9px] font-black text-slate-500 uppercase tracking-widest">Contachilometri (km)</label>
                            <input
                              type="number"
                          ...
                          <div className="col-span-2 flex items-center gap-1.5 py-0.5 select-none text-slate-650 font-sans">
                            ...
                            />
                            <label htmlFor="fuelIsFull" className="text-[11px] font-bold cursor-pointer">
                          ...
                          {/* Date */}
                          <div className="col-span-2 space-y-1">
                            <label className="block text-[9px] font-black text-slate-500 uppercase tracking-widest">Data del rifornimento</label>
                            <input
                              type="date"
                              value={expenseDate}
                              onChange={(e) => setExpenseDate(e.target.value)}
                        ...
                        <button
                          type="submit"
                          className="w-full py-2 bg-emerald-700 hover:bg-emerald-800 text-white rounded-lg text-xs font-black uppercase tracking-wider flex items-center justify-center gap-1 cursor-pointer"
                        >
                      ...
                      <div className="bg-emerald-50/40 border border-emerald-100/50 p-3 rounded-xl grid grid-cols-2 gap-2 text-sans select-none">
                        <div className="col-span-2 border-b border-emerald-100/30 pb-1.5 mb-1 flex items-center gap-1">
                          <span className="text-[10px] font-black text-emerald-800 uppercase tracking-widest">📊 Prestazioni & Consumo Medio</span>
                        </div>
                        
                        <div className="p-2 bg-white rounded-lg border border-slate-100 shadow-xs text-center">
                          <span className="text-[8px] font-black text-slate-400 uppercase block">Consumo Camper</span>
                          <span className="text-xs font-black text-emerald-800 font-mono block mt-0.5">
                            {fuelStats.kmPerLiter ? `${fuelStats.kmPerLiter.toFixed(1)} km/L` : '---'}
                          </span>
                          {fuelStats.kmPerLiter && (
                            <span className="text-[8px] text-slate-400 font-mono font-bold">
                              ({(100 / fuelStats.kmPerLiter).toFixed(1)} L/100km)
                            </span>
                          )}
                        </div>

                        <div className="p-2 bg-white rounded-lg border border-slate-100 shadow-xs text-center">
                          <span className="text-[8px] font-black text-slate-400 uppercase block">Speso Carburante</span>
                          <span className="text-xs font-black text-[#A45C40] font-mono block mt-0.5">
                            {fuelStats.totalFuelCost.toFixed(2)} €
                          </span>
                        ...
                        {fuelStats.avgPricePerLiter > 0 && (
                          <div className="p-1.5 bg-stone-100/30 rounded-lg text-center text-slate-650 font-bold text-[9px] col-span-2">
                            Prezzo medio ponderato alla pompa: <span className="font-mono text-emerald-700">{fuelStats.avgPricePerLiter.toFixed(3)} €/L</span>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="space-y-2 max-h-[180px] overflow-y-auto pr-1">
                        {activeTrip.expenses.filter(e => e.category === 'Carburante').length === 0 ? (
                          <p className="text-xs text-slate-400 py-4 text-center">Nessun rifornimento registrato.</p>
                        ) : (
                          activeTrip.expenses
                            .filter(e => e.category === 'Carburante')
                            .map((exp) => {
                              const brandColor = 
                                exp.fuelCompany === 'Eni' ? 'bg-yellow-400 text-slate-900' :
                                exp.fuelCompany === 'Q8' ? 'bg-blue-600 text-white' :
                                exp.fuelCompany === 'Esso' ? 'bg-red-500 text-white' :
                                exp.fuelCompany === 'IP' ? 'bg-green-600 text-white' :
                                exp.fuelCompany === 'Tamoil' ? 'bg-emerald-600 text-white' :
                                exp.fuelCompany === 'Coop' ? 'bg-red-700 text-white' :
                                'bg-slate-100 text-slate-700';
                                
                              return (
                                <div key={exp.id} className="p-2.5 bg-white border border-slate-100 rounded-lg hover:border-slate-200 transition-all font-sans relative group">
                                  <div className="flex justify-between items-start gap-2">
                                    <div className="space-y-0.5">
                                      <div className="flex items-center gap-1.5">
                                        <span className={`text-[8px] font-black uppercase px-2 py-0.5 rounded-full ${brandColor} font-mono`}>
                                      ...
                                      {exp.liters && exp.pricePerLiter ? (
                                        <div className="text-[10px] text-slate-500 font-semibold font-mono space-y-0.5">
                                          <div>Erogato: <b className="text-slate-800">{exp.liters.toFixed(2)} Litri</b> @ {exp.pricePerLiter.toFixed(3)} €/L</div>
                                          {exp.odometer && (
                                            <div className="text-[9px] text-[#3E4A35] font-bold">Chilometri segnati: {exp.odometer.toLocaleString()} km</div>
                                          )}
                                        </div>
                                      ) : (
                  ...
                  {activeTrip.expenses.length > 0 && (
                    <div className="p-3 bg-[#F2EFE9]/40 border border-slate-200/60 rounded-xl space-y-2 text-sans select-none mt-3 animate-fade-in">
                      <div className="flex justify-between items-center">
                        ...
                        <span className="text-[10px] font-black text-[#A45C40] font-mono bg-[#A45C40]/10 px-1.5 py-0.5 rounded">Tot: {totalExpensesOfActive.toFixed(2)}€</span>
                      </div>
                      <div className="space-y-2">
                        {(['Carburante', 'Autostrada', 'Cibo', 'Sosta', 'Altro'] as DiaryExpense['category'][]).map(cat => {
                          const spent = activeTrip.expenses.filter(e => e.category === cat).reduce((sum, e) => sum + e.amount, 0);
                          const percentage = totalExpensesOfActive > 0 ? (spent / totalExpensesOfActive) * 100 : 0;
                          if (spent === 0) return null;

                          let bgColor = 'bg-stone-400';
                          if (cat === 'Carburante') { bgColor = 'bg-emerald-600'; }
                          else if (cat === 'Autostrada') { bgColor = 'bg-cyan-500'; }
                          else if (cat === 'Cibo') { bgColor = 'bg-amber-500'; }
                          else if (cat === 'Sosta') { bgColor = 'bg-[#A45C40]'; }
                          ...
                          return (
                            <div key={cat} className="space-y-0.5">
                              <div className="flex justify-between text-[9px] font-black text-slate-600">
                                ...
                                <span className="font-mono">{spent.toFixed(2)}€ ({Math.round(percentage)}%)</span>
                              </div>
                ...
                <div className="space-y-4">
                  ...
                  <form onSubmit={handleAddPhoto} className="p-4 bg-stone-50 rounded-xl border border-stone-100 space-y-3 font-sans">
                        ...
                        >
                          ...
                          {isUploading ? (
                            <div className="space-y-2 flex flex-col items-center">
                              ...
                              <p className="text-[10px] text-slate-500 font-bold font-sans">Caricamento in corso...</p>
                            </div>
                          ) : uploadedImageUrl ? (
                            <div className="space-y-1.5 flex flex-col items-center">
                              <div className="relative w-16 h-12 rounded-lg overflow-hidden border border-emerald-500 shadow-xs">
                                <img
                                  ...
                                  referrerPolicy={uploadedImageUrl?.startsWith('http') ? "no-referrer" : undefined}
                               />
                                <div className="absolute inset-0 bg-emerald-500/25 flex items-center justify-center">
                                  <span className="text-white text-[9px] font-black">✓</span>
                                </div>
                              </div>
                              <p className="text-[10px] text-emerald-600 font-black">Foto caricata con successo!</p>
                              <p className="text-[9px] text-[#3E4A35] underline cursor-pointer font-bold">Cambia foto</p>
                            </div>
                          ) : (
                            <div className="space-y-1">
                              <Upload className="w-5 h-5 text-slate-400 mx-auto" />
                              <p className="text-[10.5px] text-slate-600 font-bold">
                                Trascina qui la foto dallo smartphone/PC o <span className="text-[#3E4A35] underline font-bold">sfoglia</span>
                              </p>
                              <p className="text-[9px] text-slate-400">PNG, JPG, WEBP fino a 15MB</p>
                            </div>
                          )}
                        </div>
                      </div>
                  ...
                  <div className="grid grid-cols-2 gap-3 max-h-[225px] overflow-y-auto pr-1">
                    {activeTrip.photos.length === 0 ? (
                      <div className="col-span-2 text-xs text-slate-400 py-8 text-center bg-white border border-slate-100 rounded-lg">
                        ...
                        Nessuno scatto caricato. Scatta o simula la prima foto della vacanza!
                      </div>
                    ) : (
                      activeTrip.photos.map((photo, idx) => (
                        ...
                        >
                          ...
                          <div className="p-1.5">
                            <p className="text-[10px] text-slate-700 leading-tight line-clamp-2">{photo.description}</p>
                          </div>
                          ...
                          <button
                            ...
                            className="absolute top-1.5 right-1.5 p-1 bg-black/60 hover:bg-red-650 text-white hover:text-white rounded-lg opacity-0 group-hover:opacity-100 transition-opacity z-10"
                            title="Rimuovi foto"
                          >
      ...
      {selectedLightboxPhotoIndex !== null && activeTrip && activeTrip.photos[selectedLightboxPhotoIndex] && (
        <div 
          ...
          >
            ...
            <div className="relative w-full aspect-video md:aspect-[4/3] bg-black flex items-center justify-center group overflow-hidden">
              <img 
                ...
                alt={activeTrip.photos[selectedLightboxPhotoIndex].description} 
                referrerPolicy={activeTrip.photos[selectedLightboxPhotoIndex].url?.startsWith('http') ? "no-referrer" : undefined}
              ...
              {activeTrip.photos.length > 1 && (
                <>
                  ...
                  <button 
                    onClick={(e) => {
                      e.stopPropagation();
                      const newIdx = (selectedLightboxPhotoIndex - 1 + activeTrip.photos.length) % activeTrip.photos.length;
                  ...
                  <button 
                    onClick={(e) => {
                      e.stopPropagation();
                      const newIdx = (selectedLightboxPhotoIndex + 1) % activeTrip.photos.length;
                      setSelectedLightboxPhotoIndex(newIdx);
                    }}
            ...
            <div className="w-full bg-stone-950 p-5 border-t border-stone-855 text-stone-200 text-left space-y-1.5 font-sans">
              <div className="flex justify-between items-center text-[10px] text-stone-400 font-bold font-mono">
                <span className="flex items-center gap-1">
                  ...
                  Scattata il: {activeTrip.photos[selectedLightboxPhotoIndex].date}
                </span>
                <span>Foto {selectedLightboxPhotoIndex + 1} di {activeTrip.photos.length}</span>
              </div>
      ...
      {showDeleteConfirm && activeTrip && (
        ...
        >
          ...
          >
            ...
            <div className="space-y-1">
              <h3 className="text-base font-black text-stone-900">Elimina questo Viaggio?</h3>
              <p className="text-xs text-stone-500 leading-relaxed">
                Sei sicuro di voler eliminare definitivamente il viaggio <span className="font-bold text-stone-855 italic">&quot;{activeTrip.title}&quot;</span>? 
                Questa azione rimuoverà permanentemente tutte le tappe, spese e scatti fotografici inseriti.
              </p>
            </div>
