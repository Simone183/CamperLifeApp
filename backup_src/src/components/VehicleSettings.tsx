    ...
    <div id="vehicle-settings" className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-1">
              Massa a pieno carico (tonnellate)
            </label>
        ...
        {(isTooTall || isHeavy || isVeryLong) && (
          <div className="p-4 bg-amber-50/70 border border-amber-200 rounded-xl space-y-2">
            ...
            <ul className="list-disc pl-5 text-[11px] text-amber-700 space-y-1 leading-relaxed">
              {isTooTall && (
                <li>
                  <strong>Altezza ({localDims.height}m) ≥ 3.0 m:</strong> Massima attenzione ai ponti ferroviari storici, passaggi coperti nel centro città e ai rami bassi nelle aree sosta naturali.
                </li>
              )}
              {isHeavy && (
                <li>
                  <strong>Peso ({localDims.weight}t) &gt; 3.5 t:</strong> Richiede patente C o superiore. Molte strade secondarie montane e ponti impongono un divieto assoluto oltre le 3.5t.
                </li>
              )}
              {isVeryLong && (
                <li>
                  <strong>Lunghezza ({localDims.length}m) ≥ 7.0 m:</strong> Ampio raggio di sterzata richiesto. Prestare grande cura nei tornanti montani alpini e nelle piccole rotatorie.
                </li>
              )}
            </ul>
          </div>
