...
export function CamperSecurityTab({ currentUser, userLocation }: { currentUser: any; userLocation: [number, number] | null }) {
  ...
  const lat = userLocation ? userLocation[0].toFixed(6) : '41.902782';
  const lng = userLocation ? userLocation[1].toFixed(6) : '12.496366';
  ...
  const generatedSOSTemplate = React.useMemo(() => {
    ...
    return `${header}\n\nTipo: ${cause}\n\n${locMessage}\n\nInviato tramite la Console Sicurezza Attiva CamperLife.`;
