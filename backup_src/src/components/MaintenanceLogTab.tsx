...
export function MaintenanceLogTab() {
  ...
  const [logs, setLogs] = React.useState<MaintenanceLog[]>(() => {
    ...
    if (saved) {
      try { return JSON.parse(saved); } catch (e) { return DEFAULT_LOGS; }
    }
    return DEFAULT_LOGS;
  });
      ...
      <div className="bg-gradient-to-br from-[#3E4A35] to-[#2B3523] rounded-3xl p-6 text-white shadow-xl relative overflow-hidden">
        ...
        <div className="mt-6 pt-6 border-t border-white/10 grid grid-cols-2 md:grid-cols-4 gap-4 relative z-10 text-center md:text-left">
          ...
          <div className="bg-white/5 rounded-xl p-3 border border-white/5">
            ...
            <span className="text-xl font-mono font-black text-rose-300 leading-tight block text-center mt-2">€{totalInvestment}</span>
          </div>
            ...
            <form onSubmit={handleAddLog} className="space-y-3">
              ...
              <div className="grid grid-cols-3 gap-3">
                <div className="col-span-1 space-y-1">
                  <label className="block text-[9px] uppercase font-bold text-slate-500">Costo (Euro)</label>
                  <input
                    type="number"
                    min="0"
                    value={newCost}
                ...
                <div className="col-span-2 space-y-1">
                  <label className="block text-[9px] uppercase font-bold text-slate-500">Note & Descrizione Dettagliata</label>
                  <input
                    type="text"
                    placeholder="Es. Sostituito guarnizione oblò con sika..."
                    value={newDesc}
