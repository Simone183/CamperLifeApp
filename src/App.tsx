...
export default function App() {
  ...
  const handleDeleteCrashReport = async (id: string) => {
    try {
      ...
      console.error("Delete crash report error:", err);
    }
  };
  ...
  const handleClearAllCrashReports = async () => {
    ...
    try {
      ...
      console.error("Clear all crash reports error:", err);
    }
  };
                    ...
                    <div className="bg-white p-5 rounded-2xl border border-rose-100 shadow-xs space-y-4">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-rose-100">
                        <div>
                          <h4 className="font-extrabold text-rose-950 text-sm flex items-center gap-2">
                            ...
                            Segnalazioni Crash & Errori Runtime ({crashReports.length})
                          </h4>
                          <p className="text-xs text-rose-800/70 mt-0.5">
                            Log catturati automaticamente quando gli utenti riscontrano errori imprevisti nell'app.
                          </p>
                        </div>
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          ...
                          {crashReports.length > 0 && (
                            <button
                              type="button"
                              onClick={handleClearAllCrashReports}
                              className="px-3 py-1.5 bg-rose-600 hover:bg-rose-700 text-white font-bold text-xs rounded-xl transition cursor-pointer flex items-center gap-1.5 shadow-xs"
                            >
                              <Trash2 className="w-3.5 h-3.5" />
                      ...
                      {crashReports.length === 0 ? (
                        <div className="text-center py-10 bg-rose-50/30 rounded-2xl border border-dashed border-rose-200">
                          ...
                          <p className="text-sm font-bold text-slate-800">Nessun crash o errore registrato!</p>
                          <p className="text-xs text-slate-500 max-w-sm mx-auto mt-1">
                            L'applicazione funziona regolarmente. Se si verificheranno errori runtime su dispositivi utenti, appariranno qui in tempo reale.
                          </p>
                        </div>
                      ) : (
                        <div className="space-y-3">
                          {crashReports.map((report) => (
                            <div
                              key={report.id}
                              className="p-4 bg-slate-900 text-slate-100 rounded-xl border border-slate-800 shadow-sm space-y-2 font-sans"
                            >
                              <div className="flex items-start justify-between gap-3">
                                <div className="space-y-1">
                                  <div className="flex items-center gap-2">
                                    <span className="px-2 py-0.5 bg-rose-500/20 text-rose-300 font-bold text-[10px] rounded-md border border-rose-500/30">
                                      CRASH
                                    </span>
                                    <span className="text-xs font-semibold text-slate-300">
                                      Utente: <strong className="text-emerald-400">{report.userEmail || "Anonimo"}</strong>
                                    </span>
                                    <span className="text-[10px] text-slate-500">
                                      {report.timestamp ? new Date(report.timestamp).toLocaleString("it-IT") : "N/D"}
                                    </span>
                                  </div>

                                  <h5 className="font-mono text-xs text-rose-300 font-bold leading-snug">
                                    {report.message}
                                  </h5>
                                </div>
...
Per favore analizza questo bug nel codice della nostra applicazione e applica la correzione.`;
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-[10px] text-slate-400 font-mono bg-slate-950/60 p-2.5 rounded-lg border border-slate-800/80">
                                <div>
                                  <span className="text-slate-500 block">URL / Sezione:</span>
                                  <span className="text-slate-300 truncate block">{report.url || "N/D"}</span>
                                </div>
                                <div>
                                  <span className="text-slate-500 block">Dispositivo / UserAgent:</span>
                                  <span className="text-slate-300 truncate block">{report.userAgent || "N/D"}</span>
                                </div>
                              </div>

                              {report.stack && (
                                <details className="group">
                                  <summary className="text-[10px] text-amber-400 hover:text-amber-300 cursor-pointer font-semibold py-1">
                                    Mostra Stack Trace Dettagliato ➔
                                  </summary>
                                  <pre className="mt-2 p-3 bg-black/90 text-slate-300 text-[10px] font-mono rounded-lg overflow-x-auto max-h-48 whitespace-pre-wrap border border-slate-800">
                                    {report.stack}
                                    {report.componentStack && `\n\nComponent Stack:\n${report.componentStack}`}
                                  </pre>
                                </details>
                    ...
                    {aiImportedSuccessList.length > 0 && (
                      ...
                      >
                        ...
                        <p className="text-[11px] sm:text-xs text-emerald-700 leading-relaxed">
                          ...
                          web su Camperonline, Park4night, Campercontact, area-sosta-camper, CaraMaps, Campermaps, ViaCamper, Arianna e Camperpass, sono ora persistiti
