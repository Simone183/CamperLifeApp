...
import { MessageSquare, Heart, Send, AlertOctagon, PlusCircle, HelpCircle, User, CheckCircle, Trash2 } from 'lucide-react';
...
export default function CommunityTab({ messages, onChange, isAdmin }: CommunityTabProps) {
  ...
  const simulateReply = (msgId: string, type: 'SOS_RESP' | 'GEN_RESP') => {
    ...
    onChange(
      messages.map((m) => {
        if (m.id === msgId) {
          // If already resolved, don't simulate replies
  ...
  const handleCreateReply = (msgId: string) => {
    ...
    if (targetMsg?.isResolved) return; // Non consentire risposte se risolto
              ...
              filteredMessages.map((msg) => (
                <div
                  ...
                  className={`rounded-2xl border p-5 space-y-4 shadow-sm transition-all ${
                    msg.tag === 'SOS'
                      ? msg.isResolved
                        ? 'border-green-300 bg-green-50/50 dark:bg-green-950/10 dark:border-green-900/40'
                ...
                >
                  ...
                  <div className="flex justify-between items-start gap-3">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className={`font-bold ${msg.tag === 'SOS' ? (msg.isResolved ? 'text-green-950 dark:text-green-200' : 'text-red-950 dark:text-red-200') : 'text-black dark:text-slate-200'} text-sm`}>{msg.user}</div>
                    ...
                    <div className="flex items-center gap-2">
                      <span className={`px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-wider ${getTagStyle(msg.tag, msg.isResolved)}`}>
                        {msg.tag === 'SOS' && msg.isResolved ? 'SOS RISOLTO' : msg.tag}
                      </span>
                      {isAdmin && (
                        <button
                          onClick={() => handleDeleteMessage(msg.id)}
                          title="Elimina Messaggio (Moderatore)"
                          className="p-1.5 bg-red-100 hover:bg-red-200 text-red-700 dark:bg-red-950/40 dark:text-red-400 dark:hover:bg-red-900/60 rounded-lg transition-all cursor-pointer flex items-center justify-center border border-red-200 dark:border-red-900/50"
                        >
                  ...
                  {/* Body Text */}
                  <p className={`text-sm ${msg.tag === 'SOS' ? (msg.isResolved ? 'text-green-950 dark:text-green-200/90' : 'text-red-950 dark:text-red-200') : 'text-black dark:text-slate-200'} leading-relaxed whitespace-pre-wrap font-medium`}>
                    {msg.text}
                  </p>

                  {/* Likes and Reply trigger */}
                  <div className="flex justify-between items-center border-y border-slate-100 dark:border-slate-700 py-2.5">
                    ...
                    {msg.tag === 'SOS' && !msg.isResolved && (
                      <button
                        onClick={() => handleResolve(msg.id)}
                        className="px-3 py-1.5 bg-green-600 hover:bg-green-700 active:bg-green-800 dark:bg-green-700 dark:hover:bg-green-600 text-white font-bold text-[10px] uppercase tracking-wider rounded-lg transition-all cursor-pointer flex items-center gap-1 shadow-sm"
                      >
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              {isAdmin && (
                                <button
                                  ...
                                  title="Elimina risposta (Moderatore)"
                                  className="text-red-605 hover:text-red-755 dark:text-red-400 dark:hover:text-red-300 p-0.5 cursor-pointer rounded hover:bg-red-50 dark:hover:bg-red-950/25 transition-all flex items-center justify-center"
                                >
                                  <Trash2 className="w-3.5 h-3.5" />
                                </button>
                              )}
                  ...
                  {/* Add Reply Inline */}
                  {msg.isResolved ? (
                    <div className="flex items-center gap-2 p-3 bg-green-50/75 dark:bg-green-950/20 border border-green-200 dark:border-green-900/30 rounded-xl text-green-800 dark:text-green-300 text-xs font-semibold">
                      <CheckCircle className="w-4 h-4 shrink-0 text-green-600 dark:text-green-400" />
                      <span>Questo SOS è stato dichiarato risolto. Non sono consentite ulteriori risposte.</span>
                    </div>
                  ) : (
