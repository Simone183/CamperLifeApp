...
export function PantryShoppingTab({ onOpenCrewModal }: { onOpenCrewModal?: () => void } = {}) {
  ...
  const [shopCat, setShopCat] = React.useState<string>('freschi');
  ...
  React.useEffect(() => {
    ...
    const unsubscribe = onSnapshot(docRef, (doc) => {
      ...
      console.error("PantryShoppingTab Firestore sync error:", error);
      setLoadedFromFirestore(true);
    });
    return unsubscribe;
  }, []);
  ...
  const saveToFirestore = (newPantry: PantryItem[], newShoppingList: ShoppingItem[]) => {
    ...
    const cleanedShoppingList = JSON.parse(JSON.stringify(newShoppingList));
    setDoc(docRef, { pantry: cleanedPantry, shoppingList: cleanedShoppingList }, { merge: true });

    // Also sync to Family Crew
    if (currentCrew && isModuleSynced('pantry')) {
      syncCrewSection('pantry', { pantry: cleanedPantry, shoppingList: cleanedShoppingList }).catch(() => {});
    }
  };
  ...
  React.useEffect(() => {
    ...
    saveToFirestore(pantry, shoppingList);
  }, [pantry, shoppingList, loadedFromFirestore]);
  ...
  return (
    <div id="pantry-shopping-container" className="space-y-6 font-sans">
      ...
      {/* Family Crew Tab Banner */}
      <FamilyCrewTabBanner moduleName="Cambusa & Lista Spesa" onOpenCrewModal={onOpenCrewModal} />
